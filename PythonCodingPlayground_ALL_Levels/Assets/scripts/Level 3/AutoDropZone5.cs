...
public class AutoDropZone5 : MonoBehaviour, IDropHandler
{
        ...
        if (draggable != null && !draggable.IsLocked)
        {
            ...
            if (CheckCorrectness(draggable.gameObject))
            {
                ...
                if (correctPlacements == totalCorrectPlacementsNeeded)
                {
                    ...
                    // Start coroutine to activate the win panel after a delay
                    StartCoroutine(ActivateWinPanelAfterDelay(5f));
                }
            }
            else
            {
    ...
    private IEnumerator ActivateWinPanelAfterDelay(float delay)
    {
