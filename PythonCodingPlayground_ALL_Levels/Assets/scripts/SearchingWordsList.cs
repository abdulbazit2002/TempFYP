...
public class SearchingWordsList : MonoBehaviour
{
    ...
    private bool ShouldScaleDown(Vector3 targetScale)
    {
        ...
        sqaureSize.x = squareRect.rect.width * targetScale.x + offset;
        sqaureSize.y = squareRect.rect.height * targetScale.y + offset;

        var totalSqaureHeight = sqaureSize.y * _rows;

        //Make sure all the squares fit in the parent rectangle area
        if (totalSqaureHeight > parentrect.rect.height)
        {
            while (totalSqaureHeight > parentrect.rect.height)
            {
                if (TryIncreaseColumnNumber())
                {
                    totalSqaureHeight = sqaureSize.y * _rows;
                }
                else
                    return true;
            }
        }
        var totalSquareWidth = sqaureSize.x * _columns;
